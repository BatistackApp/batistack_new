...
test('cache is forgotten on operations', function () {
    ...
    Cache::shouldReceive('forget')->with('vat_rates_all')->times(2);
    Cache::shouldReceive('forget')->with('vat_rate_default')->times(2); // updated, deleted
    Cache::shouldReceive('forget')->with("core_vat_rate_{$this->vat->id}")->times(1); // updated
