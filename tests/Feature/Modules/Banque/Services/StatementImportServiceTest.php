...
it('throws QueryException if it is not an integrity constraint violation', function () {
    ...
    BankTransaction::saving(function ($tx) {
        throw new QueryException('sqlite', 'INSERT...', [], new PDOException('Fake Error', 1111));
    });

    $service = new StatementImportService;

    expect(fn () => $service->importCsv($account, $filePath))->toThrow(QueryException::class);

    // Nettoyage de l'événement et du fichier
    BankTransaction::flushEventListeners();
    BankTransaction::boot();
