...
it('can generate a draft maintenance quote', function () {
    ...
    expect(\App\Models\Commerce\CustomerQuoteItem::where('customer_quote_id', $quote->id)->count())->toBe(1);
    expect((float) \App\Models\Commerce\CustomerQuoteItem::where('customer_quote_id', $quote->id)->first()->selling_price)->toBe(250.0);
});
