...
@section('content')
    ...
    <div class="grid grid-cols-2 gap-8 mb-8">
        <section>
            ...
            @foreach($thirdParty->addresses as $address)
                <div class="mb-3 pb-2 border-b border-slate-100 last:border-0">
                    <p class="font-bold text-xs">{{ $address->type->getLabel() }} @if($address->is_default) <span class="text-blue-600">(Défaut)</span> @endif</p>
                    <p>{{ $address->street }}</p>
        ...
        <section>
            ...
            @foreach($thirdParty->contacts as $contact)
                <div class="mb-3 pb-2 border-b border-slate-100 last:border-0">
                    <p class="font-bold text-xs">{{ $contact->full_name }} @if($contact->is_primary) <span class="text-blue-600">(Principal)</span> @endif</p>
    ...
    <section class="mb-8">
        <h2 class="text-sm font-bold bg-blue-batistack text-white p-2 mb-4 uppercase">Analyse de Conformité détaillée</h2>
    ...
    <footer class="fixed bottom-0 left-0 w-full text-center text-slate-400 text-[8px] py-4 border-t border-slate-100">
        {{ $company['name'] }} - Fiche Tiers Certifiée - Page 1/1
    </footer>
@endsection
