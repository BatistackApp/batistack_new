...
*   **Notes de Frais & OCR** : Workflow complet de soumission. Moteur OCR (`GoogleCloudVisionOcrService`) intégré pour l'extraction automatique des montants et catégorisation depuis les tickets. Support natif des factures PDF multi-pages (Issue #144). L'extraction retourne une valeur **vide** (aucune donnée inventée) tant que `ocr_enabled` et la clé API Google Vision ne sont pas configurés.
