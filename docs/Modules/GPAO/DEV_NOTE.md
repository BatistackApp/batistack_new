...
*   **APS (Ordonnancement IA)** : Implémentation du service `ApsSchedulingService` qui planifie automatiquement les OFs ouverts sur les machines en priorisant selon la `delivery_date` (sans garantie de respect des délais planifiés) et la disponibilité du stock. Un bouton a été ajouté au Kanban GPAO.
