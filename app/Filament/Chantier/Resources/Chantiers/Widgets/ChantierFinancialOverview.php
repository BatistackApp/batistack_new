...
class ChantierFinancialOverview extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $bankIncome = $financials['bank_income_real'];
        $bankExpense = $financials['bank_expense_real'];
        $bankNet = $financials['bank_net_real'];
        ...
        return [
            ...
            Stat::make('Encaissements (Banque)', number_format($bankIncome, 2, ',', ' ').' €')
                ->description('Crédits bancaires affectés au chantier')
                ->descriptionIcon(Phosphor::ArrowCircleDown)
                ->color('success'),

            Stat::make('Décaissements (Banque)', number_format($bankExpense, 2, ',', ' ').' €')
                ->description('Débits bancaires affectés au chantier')
                ->descriptionIcon(Phosphor::ArrowCircleUp)
                ->color('danger'),

            Stat::make('Trésorerie nette (Banque)', number_format($bankNet, 2, ',', ' ').' €')
                ->description($bankNet >= 0 ? 'Solde positif' : 'Solde négatif')
                ->descriptionIcon($bankNet >= 0 ? Phosphor::TrendUp : Phosphor::TrendDown)
                ->color($bankNet >= 0 ? 'success' : 'danger'),
        ];
    }
}
