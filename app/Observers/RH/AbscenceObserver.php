...
class AbscenceObserver
{
    ...
    public function creating(Abscence $absence): void
    {
        ...
        if (in_array($absence->type, [\App\Enums\RH\AbsenceType::SICK_LEAVE, \App\Enums\RH\AbsenceType::WORK_ACCIDENT])) {
            ...
            if ($absence->requires_subrogation && empty($absence->ij_expected)) {
                $employee = \App\Models\RH\Employee::with('currentContract')->find($absence->employee_id);
    ...
    public function created(Abscence $absence): void
    {
        ...
        if (in_array($absence->type, [\App\Enums\RH\AbsenceType::SICK_LEAVE, \App\Enums\RH\AbsenceType::WORK_ACCIDENT])) {
            try {
                $pdfRelativePath = app(\App\Services\RH\RHDocumentService::class)->generateAttestationSalaire($absence);
